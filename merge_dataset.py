...
# --- KONFIGURASI FILE ---
FILE_LAMA = 'dataset/processed/steam_new_and_fav_v1.csv' 
FILE_BARU = 'dataset/processed/dataset_final_terbaru_safe.csv' 
FILE_HASIL = 'dataset/processed/steam_new_and_fav_final_1.csv'
...
def merge_datasets():
    ...
    df_lama = pd.read_csv(FILE_LAMA, on_bad_lines='skip', low_memory=False, encoding_errors='ignore')
    df_baru = pd.read_csv(FILE_BARU, on_bad_lines='skip', low_memory=False, encoding_errors='ignore')

    print(f"Jumlah data 1: {len(df_lama)} baris")
    print(f"Jumlah data 2: {len(df_baru)} baris")

    # 3. Menggabungkan kedua DataFrame
